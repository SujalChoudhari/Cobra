...
/// <param name="namedValues">A dictionary storing defined variables and their corresponding LLVM value references.</param>
    ...
    : CobraBaseVisitor<LLVMValueRef>
{
    ...
    private readonly Dictionary<string, LLVMValueRef> _namedValues = namedValues;
    ...
    /// based on the type of statement (declaration, assignment, or expression).
    /// </summary>
    /// <param name="context">The parse tree context representing the statement.</param>
    /// <returns>A value representing the result of visiting the statement, or a default value if no applicable handler is found.</returns>
    public override LLVMValueRef VisitStatement(CobraParser.StatementContext context)
    {
        ...
        // TODO: Add support for other statement types (e.g., control flow)
        return default;
    }
    ...
    /// <summary>
    /// Processes a declaration statement in the parsed program, assigning types and allocating memory for variables.
    /// Handles the type information and initial value assignment if provided in the declaration.
    /// </summary>
    /// <param name="context">The context of the declaration statement node in the parse tree, containing the variable name, type, and optional initializer.</param>
    /// <returns>Returns an <see cref="LLVMValueRef"/> representing the allocated variable in the LLVM IR.</returns>
    /// <exception cref="Exception">Thrown when an invalid or unsupported type is specified or if the variable is already declared.</exception>
    public override LLVMValueRef VisitDeclarationStatement(CobraParser.DeclarationStatementContext context)
    {
        ...
        {
            CobraLogger.Error($"Variable '{variableName}' is already declared.");
            throw new Exception($"Cannot declare variable. Variable '{variableName}' is already declared.");
        }

        var allocatedValue = _builder.BuildAlloca(varType, variableName);
        _namedValues[variableName] = allocatedValue;
    ...
    /// <summary>
    /// Handles the visitation of an assignment statement in the parsed abstract syntax tree (AST).
    /// Evaluates the right-hand side expression and stores its value in the left-hand side variable.
    /// Supports compound assignment operators like `+=`, `-=`, etc.
    /// </summary>
    /// <param name="context">The context for the assignment statement, providing access to variable names, operators, and expressions in the statement.</param>
    /// <returns>The resulting value stored in the left-hand side variable after evaluating the assignment.</returns>
    /// <exception cref="Exception">Thrown if the left-hand side variable is not declared before the assignment or if an unsupported operator is used.</exception>
    public override LLVMValueRef VisitAssignmentStatement(CobraParser.AssignmentStatementContext context)
    {
        ...
        // TODO: Add resolvers for member variables and ops

        if (!_namedValues.TryGetValue(variableName, out var variableAddress))
        {
            CobraLogger.Error($"Undeclared variable: '{variableName}'");
        ...
        else
        {
            ...
            CobraLogger.RuntimeVariableValue(_builder, module, $"Assigned value to variable: {variableName}:", loaded);
        }
    ...
    public override LLVMValueRef VisitUnaryExpression(CobraParser.UnaryExpressionContext context)
    {
        ...
        if (op == "!")
        {
            return _builder.BuildNot(operand, "logical_not");
        }

        if (op == "~")
        {
            return _builder.BuildNot(operand, "bitwise_not");
        }
        ...
        var addr = _namedValues[varName];
        var oldVal = _builder.BuildLoad2(LLVMTypeRef.Int32, addr, varName);
        var newVal = op == "++"
    ...
    public override LLVMValueRef VisitPostfixExpression(CobraParser.PostfixExpressionContext context)
    {
        ...
        for (var i = 0; i < context.INC().Length + context.DEC().Length; i++)
        {
            var op = context.GetChild(context.primary().ChildCount + i - 1).GetText();
    ...
    public override LLVMValueRef VisitPrimary(CobraParser.PrimaryContext context)
    {
        ...
        {
            var variableName = context.ID().GetText();
            if (_namedValues.TryGetValue(variableName, out var varRef))
            {
